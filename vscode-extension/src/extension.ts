...
function getConfig() {
    ...
    return {
        agentPath:   cfg.get<string>('agentPath', 'devops-agent'),
        projectPath: cfg.get<string>('projectPath', ''),
    };
}

function resolveProjectPath(cfg: ReturnType<typeof getConfig>): string {
...
function resolveAgent(configured: string, cwd: string): [string, string[]] {
    ...
    if (uvBin) { return [uvBin, ['run', '--project', cwd, 'devops-agent']]; }
...
// ---------------------------------------------------------------------------
const SYSTEM_PROMPT =
`You are a senior Site Reliability / DevOps engineer acting as an on-call triage assistant.
